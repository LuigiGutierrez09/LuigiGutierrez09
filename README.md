
<a href="https://github.com/LuigiGutierrez09">![LuigiGutierrez09 Profile GIF](#)</a>

<!-- Social icons section -->
<h1 align="center"> Social Media: </h1>
  <p align="center">
    <a href="#">
      <img src="https://img.shields.io/badge/stack%20overflow-7cebf5?&style=for-the-badge&logo=stack%20overflow&logoColor=black">
    </a>
    <a href="#">
      <img src="https://img.shields.io/badge/WebPage-7cebf5?&style=for-the-badge&logo=data:image/png;base64,&logoColor=black">
    </a>
    <a href="http://www.linkedin.com/in/luigi-gutierrez-salazar-a75057134">
      <img src="https://img.shields.io/badge/linkedin-7cebf5?&style=for-the-badge&logo=linkedin&logoColor=black">
    </a>
  
  </p>
</h1>

<hr>

<!-- Description about me -->
<h2 align="center"> 🤔 About me 👨‍💻 </h2>

Eyyyy!!! <br />
I am <b>Luigi Gutierrez</b> better known as 'Luigi'😎 <br />
I am from <strong>Peru </strong> where there is a lot of food 👅<br />
I am a human who likes to learn things and makes his imagination fly when programming🚀<br />
<br/><br/> 


### 🧐 More About Me:
<table style="border: none;">
  <tr style="border: none;">
    <td style="border: none;">
      <ul>
        <li>
          🔭 Actualmente estoy estudiando en la **Universidad Nacional Mayor de San Marcos**
        </li>
        <li>
          📘 Constantemente estoy aprendiendo nuevas herramientas de programacion
        </li>
        <li>
          👀 Actualmente estoy aprendiendo React , Laravel y Spring
        </li>
        <li>
          👨🏻‍💻 La mayoría de mis proyectos están disponibles en [Github](https://github.com/LuigiGutierrez09?tab=repositories)
        </li>
        <li>
          💬 Pregúntame sobre cualquier cosa relacionada con programacion, especialmente del lenguaje JAVA (si tienes preguntas sobre Python, puedes preguntarme 🐍);
        </li>
        <li>
          📫 Puede ponerse en contacto conmigo en mi [LinkedIn](www.linkedin.com/in/luigi-gutierrez-salazar9)
        </li>
        <li>
          💪 Cuando estoy libre, me gusta hacer deporte 
        </li>
        <li>
          📝 Puedes visitar mi pagina (https://www.davidsdvm.com/) 
        </li>
    </td>
    <td style="border: none;">
      <img align="right" alt="GIF" src="#" width="450vw"/>
    </td>
  </tr>
</table>
<br><br><br>

<!-- languajes and skills section -->

<h1 align="center"> Lenguajes/Frameworks que Domino  || Languages/Frameworks that I am fluent in</h1>
<p align="center">
 <code><a href="https://www.java.com/en/"><img alt="Java" title="Java" src="./assets/java.png" height="42"></a></code>
  <code><a href="https://www.python.org/"><img alt="Python" title="Python" src="./assets/python.png" height="42"></a></code>

  <code><a href="https://getbootstrap.com"><img alt="Bootstrap" title="Bootstrap" src="./assets/Bootstrap_logo.png" height="42"></a></code>
  <code><a href="https://www.w3.org/Style/CSS/Overview.en.html"><img alt="CSS 3" title="CSS 3" src="./assets/css.png" height="42"></a></code>
  <code><a href="https://github.com/features/actions"><img alt="GitHub Actions" title="GitHub Actions" src="./assets/actions.png" height="42"></a></code>
  <code><a href="https://en.wikipedia.org/wiki/HTML"><img alt="HTML 5" title="HTML 5" src="./assets/html.png" height="42"></a></code>
  <code><a href="https://vuejs.org/"><img alt="Vue" title="Vue" src="./assets/vue.png" height="42"></a></code>
  <code><a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript"><img alt="JavaScript" title="JavaScript" src="./assets/js.png" height="42"></a></code>
  <code><a href="https://www.djangoproject.com/"><img alt="Django" title="Django" src="./assets/Django-Logo.png" height="42"></a></code>
</p>
<br>

<h1 align="center"> 📘 Lenguajes/Frameworks que estoy Aprendiendo || Languages/Frameworks I'm Learning </h1>
<p align="center">

  <code><a href="https://reactjs.org/"><img alt="ReactJS" title="ReactJS" src="./assets/react.png" height="42"></a></code>
  <code><a href="https://laravel.com/"><img alt="Laravel" title="Laravel" src="./assets/laravel.png" height="42"></a></code>
  <code><a href="https://spring.io/"><img alt="Spring" title="Spring" src="./assets/spring-logo.png" height="42"></a></code>
 
</p>
<br>

<h1 align="center"> Entornos con los que trabajo || Environments I work with </h1>
<p align="center">
 <code><a href="https://code.visualstudio.com/"><img alt="Vs code" title="Vs code" src="./assets/vscode.png" height="42"></a></code>
  <code><a href="https://github.com/"><img alt="GitHub" title="GitHub" src="./assets/github.png" height="42"></a></code>
 
</p>
<br>


<br>

<!-- last refresh of readme section -->

Last refresh: <b>Tuesday, December 20, 7:17 AM GMT-5</b>


